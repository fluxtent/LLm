...
def render_personalized_response(plan: ResponsePlan) -> str:
    ...
    if understanding.user_intent == "next_step":
        return _pick(
            ...
            [
                (
                    f"Start smaller than your life. If {latest_fragment} is the whole feeling, the next move is to name the one thing in front of you right now. "
                    "Write one sentence: \"The immediate problem is...\" Then handle only that sentence."
                ),
                (
                    f"When everything feels like {latest_fragment}, your brain is asking for one foothold, not a master plan. "
                    "Look around and choose the next physical action: sit down, drink water, open the message, close the tab, or write the one thing you are avoiding."
                ),
                (
    ...
    if understanding.distress_level >= 1:
        ...
        return _pick(
            ...
            [
                (
                    f"The real thing here is {topic}, and it sounds personal, not theoretical.{memory} "
                    "The next useful move is to separate the feeling from the trigger: what happened right before it got loud?"
                ),
                (
                    f"I would not treat {topic} like a generic mood. Something in it is asking to be understood specifically.{memory} "
                    "Start with the moment it changed: what happened, what did you tell yourself it meant, and what did you do next?"
                ),
                (
                    f"{topic.capitalize()} sounds like the center of this. I would slow it down rather than talk over it.{memory} "
                    "The most helpful next detail is the last concrete thing that made the feeling spike."
                ),
            ],
        )

    if understanding.mode == "health":
        return _health_response(plan, topic)

    if understanding.mode == "portfolio":
        return _portfolio_response(plan, topic)

    if understanding.user_intent == "planning":
        return _pick(
            plan,
            [
                f"Here is the simple version for {topic}: decide the goal, protect the constraint, then choose the next action that keeps you from losing ground.",
                f"For {topic}, make the plan around one question: what move helps you progress without creating a mess you cannot undo?",
                f"The plan for {topic} should start with the deadline and the risk. Once those are clear, the first step is usually obvious.",
            ],
        )

    if understanding.user_intent == "comparison":
        return _pick(
            plan,
            [
                f"Compare {topic} by asking which option gives you the most upside without trapping you if you change your mind.",
                f"The better choice in {topic} is probably the one that protects your most limited resource: time, money, energy, reputation, or health.",
                f"For {topic}, ignore which option sounds best for a second. Which one is reversible, affordable, and aligned with what you actually want next?",
            ],
        )

    if understanding.user_intent == "creation":
        return _pick(
            plan,
            [
                f"I can make {topic}, but the useful version needs a target: who it is for, how polished it should be, and what it has to accomplish.",
                f"For {topic}, I would start with a rough draft, then tighten tone and structure after the shape is visible.",
                f"Give me the format for {topic} and I will produce the actual thing, not just advice about it.",
            ],
        )

    if understanding.user_intent == "debugging":
        return _pick(
            plan,
            [
                f"For {topic}, start with the exact mismatch: what you expected, what happened, and the first place those diverge.",
                f"Debug {topic} by shrinking it until the failure has nowhere to hide. One input, one output, one suspected cause.",
                f"The fastest way through {topic} is not guessing. Send the error or broken behavior and isolate the smallest case that still fails.",
            ],
        )

    if understanding.direct_question:
        return _answer_direct_question(plan)
...
def _assistant_repair_response(plan: ResponsePlan) -> str:
    latest = _normalize(plan.understanding.latest_user)
    prior_need = _prior_user_from_context(plan.prior_context)
    if "simplify" in latest or "what are you talking about" in latest:
        if prior_need:
            if "purpose" in _normalize(prior_need):
                return "Plain version: purpose means what something is for, or why it matters. A hammer is for building. A choice matters because it changes your life. A person’s purpose is usually something they build through what they care about and keep choosing."
            return (
                f"Plain version: {prior_need} needs a real answer, not a wrapper. "
                "I overcomplicated it. Say the actual problem in one short sentence and I will answer that sentence directly."
            )
        return "Plain version: I overcomplicated the answer. I should give you the direct point first, then only add detail if it helps."

    if "repeat" in latest or "repeating" in latest or "same format" in latest:
        return (
            "Yes. That was repetition, not intelligence. I was wrapping your words in the same shell instead of answering them. "
            "The fix is to respond to the meaning of the message and the prior turn, not mirror the sentence structure back at you."
        )

    if prior_need:
        return (
            f"You're right to call that out. I missed the actual situation: {prior_need}. "
            "I should have answered the pain directly instead of giving you a detached framework. "
            "First, are you safe right now? If yes, tell me the one part that feels most urgent and I will stay on that."
        )
...
def _answer_direct_question(plan: ResponsePlan) -> str:
    text = _normalize(plan.understanding.latest_user)
    topic = _user_facing_fragment(plan.understanding.topic)
    ...
    if "purpose" in text:
        return _pick(
            ...
            [
                (
                    "Purpose is not a hidden label stamped onto everything. It is the role something plays: what it changes, who it serves, what it makes possible, or why someone chooses to keep caring about it."
                ),
                (
                    "The purpose of anything is the reason it matters in context. A tool has purpose because it does something; a choice has purpose because it moves a life; a person builds purpose through what they care for and commit to."
                ),
                (
                    "Purpose usually means direction plus value. Something has purpose when it points toward an outcome that matters, even if that meaning is made rather than discovered."
                ),
            ],
        )

    if "meaning" in text and ("life" in text or "anything" in text):
        return _pick(
            plan,
            [
                "Meaning is what makes an experience feel worth carrying. It can come from love, responsibility, creation, faith, curiosity, repair, or even refusing to give up on something that matters.",
                "Meaning is not always a grand answer. Sometimes it is the pattern of what you keep choosing when things are hard: who you protect, what you build, what you learn, and what you refuse to abandon.",
            ],
        )
    ...
    if text.startswith("why"):
        return _pick(
            plan,
            [
                f"Because {topic} has some cause, pressure, or tradeoff underneath it. The way to understand it is to identify what changed, what constraint appeared, and what effect followed.",
                f"The likely reason behind {topic} is not one magic cause; it is probably a chain. Find the first thing that changed, then the reaction it created.",
            ],
        )
    ...
    if text.startswith("how"):
        return _pick(
            plan,
            [
                f"The way {topic} works is by breaking it into mechanism and outcome: what happens first, what it triggers, and what changes because of it.",
                f"To understand {topic}, trace the steps instead of the label. Start with the input, then the process, then the result.",
            ],
        )

    if text.startswith(("what is", "what's", "define")):
        cleaned = re.sub(r"^(what is|what's|define)\s+", "", plan.understanding.latest_user, flags=re.I).strip(" ?")
        subject = _user_facing_fragment(cleaned or topic)
        return f"{subject.capitalize()} is the thing or idea you are asking to pin down. In simple terms, define it by what it does, where it shows up, and why it matters in the situation you care about."

    return _pick(
        plan,
        [
            f"Short answer: {topic} needs a direct answer tied to context, not a generic explanation. The key is what outcome you are trying to get from the answer.",
            f"The direct answer depends on the role {topic} plays in your situation. If it is a decision, name the constraint; if it is confusion, name the missing piece.",
            f"I would answer {topic} by first separating the actual question from the surrounding noise, then giving the simplest true version.",
        ],
    )


def _health_response(plan: ResponsePlan, topic: str) -> str:
    return _pick(
        plan,
        [
            f"With {topic}, the safe answer is general: timing, severity, changes, and accompanying symptoms matter most. If it is sudden, severe, worsening, or affecting daily life, get medical care.",
            f"For {topic}, I would not jump to a diagnosis. Track when it started, how intense it is, what makes it better or worse, and whether anything else is happening with it.",
            f"{topic.capitalize()} can mean different things depending on context. If it feels urgent or abnormal for you, a clinician is the right next step; I can help organize what to tell them.",
        ],
    )


def _portfolio_response(plan: ResponsePlan, topic: str) -> str:
    return _pick(
        plan,
        [
            f"MedBrief's answer to {topic} should be concrete: it is a custom-model chat product with memory, safety handling, API keys, and response planning built around personal context.",
            f"The product point is not just chat UI. MedBrief is meant to behave like an assistant that remembers context, adapts tone, and serves its own model through an OpenAI-style API.",
            f"For {topic}, the important distinction is ownership: MedBrief should run its own backend and generated keys, while the interface feels as fluid as a modern assistant.",
        ],
    )


def _memory_clause(plan: ResponsePlan) -> str:
    if not plan.relevant_memory:
        return ""
    memory = ", ".join(plan.relevant_memory[:2])
    return f" I am keeping the relevant memory in view: {memory}."
...
def _pick(plan: ResponsePlan, options: list[str]) -> str:
    if not options:
        return ""
    material = "||".join(
        [
            plan.understanding.latest_user,
            plan.understanding.user_intent,
            plan.understanding.emotional_state,
            str(len(plan.understanding.previous_assistant_messages)),
            "||".join(plan.prior_context),
        ]
    )
